...
lives in [README.md](README.md); development guidance lives in
...
64-bit EUI, 128-bit Microchip serial, or 128-bit ST UID. The RTC EUI is a
...
3. **Unlocked internals** — read/write/update logic shared by the public API.
   ...
   header page carrying the magic byte last, then verifies by read-back.
   On blank EEPROMs, failure before the final header write leaves magic
   unprogrammed. Forced overwrites are not atomic: the old header remains
   until the final write, and power loss during a page write can leave
   partial data.
...
CI covers ESP-IDF 5.2.6 on ESP32 and 5.5.4 on all declared targets. These
checks do not substitute for validation on an assembled board.
